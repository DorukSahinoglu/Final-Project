...
class OutputTraceCollector:
    def __init__(self, addresses: List[Dict], vehicles: List[Dict], sample_every: int = 10):
        self.addresses = addresses
        self.vehicles = vehicles
        self.sample_every = max(1, int(sample_every))
        self._wolf_iterations: Dict[int, List[Dict]] = {}

    def _routes_payload(self, problem: "HCVRPProblem", state: "SolutionState") -> List[Dict]:
        vehicle_ids_by_type = getattr(problem, "output_vehicle_ids_by_type", {})
        type_counts: Dict[int, int] = {}
        payload: List[Dict] = []
        for type_id, route in zip(state.vehicle_ids, state.routes):
            idx = type_counts.get(type_id, 0)
            type_counts[type_id] = idx + 1
            candidates = vehicle_ids_by_type.get(type_id, [])
            vehicle_id = candidates[idx] if idx < len(candidates) else type_id
            payload.append({
    ...
    def snapshot(
            ...
            stage: str,
            hunt: int,
            explore_iter: Optional[int] = None
    ) -> None:
...
def free_search_phase(
        ...
        trace_collector: Optional[OutputTraceCollector] = None,
        hunt_idx: int = 0,
):
    ...
    for step in range(iterations_per_wolf):
        ...
        if trace_collector is not None and (step + 1) % trace_collector.sample_every == 0:
            for wolf_id, wolf in enumerate(wolves):
                trace_collector.snapshot(
                    wolf.problem,
                    wolf.state,
                    wolf_id=wolf_id,
                    stage="explore",
...
def _build_placeholder_addresses(n_nodes: int) -> List[Dict]:
    rows: List[Dict] = []
    for idx in range(n_nodes):
        rows.append({
            "id": idx,
            "label": "Depo" if idx == 0 else f"Müşteri {idx}",
            "address": "",
            "isDepot": idx == 0,
        })
    return rows
...
def _build_coords_from_addresses(addresses: List[Dict]) -> List[Point]:
    coords: List[Point] = []
    for idx, row in enumerate(addresses):
        ...
        if lat is not None and lon is not None:
            ...
            coords.append((float(idx), 0.0))
    return coords


def _parse_vehicle_rows(vehicle_rows: List[Dict]) -> Tuple[List[Vehicle], List[Dict], Dict[int, List[int]]]:
    vehicles: List[Vehicle] = []
    output_rows: List[Dict] = []
    grouped_ids: Dict[Tuple[float, float, float, float], List[int]] = {}

    for raw_idx, row in enumerate(vehicle_rows):
        ...
        base_id = int(row.get("id", raw_idx))
        base_name = str(row.get("name", f"Vehicle-{base_id}"))
        base_type = str(row.get("type", "Vehicle"))
        color = row.get("color", "#00f5ff")
        key = (capacity, cost_per_km, fixed_cost, speed)

        for copy_idx in range(count):
            vehicle_id = base_id if count == 1 else base_id * 1000 + copy_idx
            vehicles.append(
                Vehicle(
            ...
            grouped_ids.setdefault(key, []).append(vehicle_id)
            output_rows.append({
                "id": vehicle_id,
                "name": base_name if count == 1 else f"{base_name}-{copy_idx + 1}",
                "type": base_type,
                "capacity": capacity,
                "color": color,
            })

    return vehicles, output_rows, grouped_ids


def build_matrix_backed_problem(payload: Dict) -> Tuple[HCVRPProblem, List[Dict], List[Dict]]:
    distance_matrix = payload.get("distance_matrix") or payload.get("distance")
    if not isinstance(distance_matrix, list) or not distance_matrix:
        raise ValueError("distance_matrix gerekli ve kare matris olmalı.")
    n_nodes = len(distance_matrix)
    if any(len(row) != n_nodes for row in distance_matrix):
        raise ValueError("distance_matrix kare olmalı.")

    time_matrix = payload.get("time_matrix") or payload.get("time")
    if time_matrix is not None:
        if len(time_matrix) != n_nodes or any(len(row) != n_nodes for row in time_matrix):
            raise ValueError("time_matrix boyutu distance_matrix ile aynı olmalı.")

    addresses = payload.get("addresses")
    if not isinstance(addresses, list) or len(addresses) != n_nodes:
        addresses = _build_placeholder_addresses(n_nodes)
    coords = _build_coords_from_addresses(addresses)

    raw_demands = payload.get("demands")
    if isinstance(raw_demands, list) and len(raw_demands) == n_nodes:
        demands = [float(x) for x in raw_demands]
    else:
        demands = [0.0] * n_nodes
        for idx, row in enumerate(addresses):
            demands[idx] = float(row.get("demand", 0.0))
    demands[0] = 0.0

    service_times = payload.get("service_times")
    if isinstance(service_times, list) and len(service_times) == n_nodes:
        service_times = [float(x) for x in service_times]
    else:
        service_times = [float(row.get("service_time", 0.0)) for row in addresses]

    time_windows = payload.get("time_windows")
    if isinstance(time_windows, list) and len(time_windows) == n_nodes:
        parsed_time_windows = []
        for row in time_windows:
            parsed_time_windows.append((float(row[0]), float(row[1])))
        ...
        for row in addresses:
            ...
            if isinstance(tw, list) and len(tw) == 2:
                parsed_time_windows.append((float(tw[0]), float(tw[1])))
            else:
                parsed_time_windows.append((0.0, float("inf")))
        time_windows = parsed_time_windows

    vehicle_rows = payload.get("vehicles")
    if not isinstance(vehicle_rows, list) or not vehicle_rows:
        raise ValueError("vehicles listesi gerekli.")
    vehicles, output_vehicle_rows, grouped_ids = _parse_vehicle_rows(vehicle_rows)
    ...
    problem.dist = [[float(x) for x in row] for row in distance_matrix]
    if np is not None:
        problem.dist_np = np.asarray(problem.dist, dtype=np.float64)
    if time_matrix is not None:
        problem.time_matrix = [[float(x) for x in row] for row in time_matrix]  # type: ignore[attr-defined]
    else:
        problem.time_matrix = None  # type: ignore[attr-defined]

    key_to_type_id = {
        (vt.capacity, vt.cost_per_km, vt.fixed_cost, vt.speed): vt.type_id
        for vt in problem.vehicle_types
    }
    output_vehicle_ids_by_type: Dict[int, List[int]] = {}
    for key, ids in grouped_ids.items():
        type_id = key_to_type_id[key]
        output_vehicle_ids_by_type[type_id] = ids[:]
    problem.output_vehicle_ids_by_type = output_vehicle_ids_by_type  # type: ignore[attr-defined]
    problem.original_vrplib_name = payload.get("name", "matrix-input")  # type: ignore[attr-defined]
    return problem, addresses, output_vehicle_rows
...
if __name__ == "__main__":
    ...
    input_path = Path(sys.argv[1]) if len(sys.argv) > 1 else base_dir / "sample_matrix_input_10.json"
    output_path = Path(sys.argv[2]) if len(sys.argv) > 2 else base_dir / "Inspection-output.json"
    ...
    user_solver_params = payload.get("solver_params")
    if isinstance(user_solver_params, dict):
        solver_params.update(user_solver_params)

    trace_every = int(payload.get("trace_every", 10))
    collector = OutputTraceCollector(addresses=addresses, vehicles=vehicles, sample_every=trace_every)
